...
describe('influxdb', () => {
    ...
    describe('init', () => {
        ...
        it('gets the names of the databases and creates a `browsertime` database if one does not already exist', async () => {

            influx.getDatabaseNames.mockResolvedValue(['database1', 'database2']);

            await init();

            expect(influx.createDatabase).toBeCalledWith('browsertime');

        });

        it('gets the names of the databases and does not create a `browsertime` database if one already exists', async () => {

            influx.getDatabaseNames.mockResolvedValue(['database1', 'browsertime']);
